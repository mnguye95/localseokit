    ...
    <div className='mx-5'>
          <div className='bg-white p-10 mx-1 border-2 border-gray-200 text-center'>
            ...
            <h6 className="lg:text-4xl md:text-2xl text-xl pb-4 font-sans font-bold text-center">Log into your account</h6>
              ...
              <p className='cursor-pointer hover:underline font-bold text-[#00df9a]' onClick={() => {setForgot(true)}}>Forgot password?</p>
            </> 
            :
            <>
              <h6 className="lg:text-4xl md:text-2xl text-xl pb-4 font-sans font-bold text-center">Forgot password?</h6>
              <form onSubmit={handleForgot} className='text-center'>
                <div className='flex flex-col py-2'>
                  <input onChange={(e) => setEmail(e.target.value)} className='border p-3 m-2' type='email' placeholder='Email Address'/>
                </div>
                <button type='success' className='bg-[#00df9a] w-full rounded-md text-lg font-medium mx-auto my-4 p-6 text-white'>Reset Password</button>
              </form>
              <p className='cursor-pointer hover:underline font-bold text-[#00df9a]' onClick={() => {setForgot(false)}}>Remembered it? Log in.</p>
              </>}
            </div>
          <div className='mt-6 text-center'>
           Don't have an account? <Link className='cursor-pointer hover:underline font-bold text-[#00df9a]' to='/register'>Create one.</Link>
          </div>
      </div>
    </div>
  )
